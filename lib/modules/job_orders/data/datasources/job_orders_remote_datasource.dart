...
class JobOrdersRemoteDataSource {
  final http.Client _client;

  JobOrdersRemoteDataSource({http.Client? client}) : _client = client ?? http.Client();

  Future<List<JobOrderModel>> getCustomerJobOrders() async {
    ...
    final uri = Uri.parse('$baseUrl${ApiEndpoints.customerJobOrders}');
    ...
    if (decoded is List) {
      return decoded
          .whereType<Map>()
          .map((e) => JobOrderModel.fromJson(Map<String, dynamic>.from(e)))
          .toList();
    }
